...
import { checkMpesaPaymentStatus, initiateMpesaPayment, isValidKenyanPhoneNumber } from "@/lib/paymentApi";
...
const PaymentFlow = ({ isOpen, onBack, onComplete }: PaymentFlowProps) => {
  ...
  const pollForConfirmation = async (activeCheckoutId: string) => {
    ...
    for (let attempt = 0; attempt < 12; attempt += 1) {
      ...
      const statusResult = await checkMpesaPaymentStatus(activeCheckoutId);
      ...
      if (statusResult.status === "failed") {
        setStatusText(null);
        setIsPaying(false);
        setErrorText(statusResult.resultDesc || "Payment failed or was cancelled. Please try again.");
        return;
      }
    ...
    setStatusText(null);
    setIsPaying(false);
    setErrorText("Payment confirmation timed out. Tap Finish Application again to re-check status.");
  };
  ...
  const handlePayment = async () => {
    ...
    if (!isValidKenyanPhoneNumber(phoneNumber)) {
      setErrorText("Enter a valid M-Pesa number (07..., 011..., 254..., or +254...).");
      return;
    }
    ...
    try {
      ...
      setStatusText("Requesting M-Pesa prompt...");

      const result = await initiateMpesaPayment({
        phoneNumber,
        amount: 130,
        description: "Application processing fee",
        referencePrefix: refundCode,
      });

      setCheckoutId(result.checkoutRequestId);
      await pollForConfirmation(result.checkoutRequestId);
    } catch (err) {
      setStatusText(null);
      setIsPaying(false);
      setErrorText(err instanceof Error ? err.message : "Failed to send STK push. Please try again.");
    }
  };
                ...
                <div className="p-6">
                  ...
                  <div className="bg-muted rounded-2xl p-4 mb-6">
                    <div className="text-center mb-4">
                      <span className="text-4xl font-display font-bold text-primary">KSH 130</span>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-center">
                      <div>
                        ...
                        <p className="font-semibold text-foreground">KSH 130</p>
                      </div>
                      <div>
                  ...
                  <div className="border border-border rounded-2xl p-4">
                    ...
                    <div className="bg-primary/5 rounded-xl p-3 mb-4 text-center">
                      <span className="text-2xl font-display font-bold text-primary">KSH 130</span>
