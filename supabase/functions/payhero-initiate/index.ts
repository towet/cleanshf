...
function getAuthHeader(): string | null {
  const token =
    Deno.env.get("PAYHERO_AUTH_TOKEN")?.trim() ||
    "Basic QTV1NEp3S0dzZ1U1VHZvSTVDN1g6UDRaMUx0UnBjalcwUkcxVnNWT3p4ZjVpTG54SzBiQnVWN0tIQ09ETw==";
  if (token) {
    return token.startsWith("Basic ") ? token : `Basic ${token}`;
  }

  const username = Deno.env.get("PAYHERO_API_USERNAME")?.trim() || "A5u4JwKGsgU5TvoI5C7X";
  const password = Deno.env.get("PAYHERO_API_PASSWORD")?.trim() || "P4Z1LtRpcjW0RG1VsVOzxf5iLnxK0bBuV7KHCODO";
...
function getChannelId(): number | null {
  const raw = Deno.env.get("PAYHERO_CHANNEL_ID")?.trim() || "11632";
  if (!raw) return null;
